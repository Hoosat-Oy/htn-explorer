...
const AddressInfo = () => {
    ...
    //         <tr className="trow">
    //             <td>Balance</td>
    //             <td>{addressBalance/100000000} HTN</td>
    //         </tr>
    //         <tr>
    //             <td>UTXOs</td>
    //             <td>{utxos ? <ul>
    ...
    //                 .map(x => <li>{x.utxoEntry.amount/100000000} HTN ({x.outpoint.transactionId})</li>)}
        ...
        <Container className="webpage addressinfo-box" fluid>
            ...
            <Row>
                <Col md={12} className="mt-sm-4">
                    ...
                    <div className="utxo-value-mono"><span class="addressinfo-color">hoosat:</span>{addr.substring(6, addr.length - 8)}<span class="addressinfo-color">{addr.substring(addr.length - 8)}</span>
            ...
            <Row>
                <Col sm={6} md={4}>
                    ...
                    <div className="utxo-value d-flex">
                        {addressBalance !== undefined ? <div className="utxo-amount">+{numberWithCommas(addressBalance / 100000000)} HTN</div> : <Spinner animation="border" variant="primary" />}</div>
                </Col>
                <Col sm={6} md={4}>
                                    ...
                                    {x.inputs?.length > 0 ? x.inputs.map(x => {
                                        return (txsInpCache && txsInpCache[x.previous_outpoint_hash]) ? <>
                                            <Row id={`N${x.previous_outpoint_hash}${x.previous_outpoint_index}`}>
                                                ...
                                                <Col xs={5}><span className="block-utxo-amount-minus">-{numberWithCommas(getAmountFromOutputs(txsInpCache[x.previous_outpoint_hash]["outputs"], x.previous_outpoint_index))}&nbsp;HTN</span></Col></Row></> : <li key={`${x.previous_outpoint_hash}${x.previous_outpoint_index}`}>{x.previous_outpoint_hash} #{x.previous_outpoint_index}</li>
                                    }) : "COINBASE (New coins)"}
                            ...
                            <Col sm={12} md={6}>
                                ...
                                <div className="utxo-value-mono" style={{ fontSize: "smaller" }}>
                                    {x.outputs.map(x => <Row>
                                        ...
                                        <Col xs={5}><span className="block-utxo-amount">+{numberWithCommas(x.amount / 100000000)}&nbsp;HTN</span></Col></Row>)}
                                </div>
                            </Col>
                            <Col md={12}>
            ...
            <Container className="webpage addressinfo-box mt-4" fluid>
                ...
                {!loadingUtxos ? utxos.sort((a, b) => b.utxoEntry.blockDaaScore - a.utxoEntry.blockDaaScore).slice((active - 1) * 10, (active - 1) * 10 + 10).map((x) => <>
                    ...
                    <Row className="utxo-border pb-4 mb-4">
                        ...
                        <Col sm={6} md={4}>
                            <div className="utxo-header mt-3">amount</div>
                            <div className="utxo-value d-flex flex-row"><div className="utxo-amount">+{numberWithCommas(x.utxoEntry.amount / 100000000)} HTN</div></div>
                        </Col>
                        <Col sm={6} md={4}>
                            <div className="utxo-header mt-3">value</div>
