...
const encodeAddress = (prefix, payload, version) => {
  ...
  let checksum_num = polymod([...Array.from(prefix).map((c) => c.charCodeAt(0) & 0x1f), 0, ...address, 0, 0, 0, 0, 0, 0, 0, 0]);
...
export const parsePayload = (payload) => {
  ...
  if (script[0] < 0x76) {
    ...
    return [encodeAddress("hoosat", address, version), String.fromCharCode(...buffer.slice(19 + length, buffer.length))];
  }
  return [payload, ""];
};
